...
def PFIRS(
    ...
    with arcpy.EnvManager(
        ...
        # AGENCY field should reflect only the names of the organizations we want to count to mas, with all others being set to "OTHER"
        ...
        calc_field_2 = arcpy.management.CalculateField(
            ...
            field="AGENCY",
            expression="get_agency(!AGENCY_!)",
            expression_type="PYTHON3",
            code_block="""def get_agency(agency):
                            return pfirs_lookup_dict[agency]
            """,
            field_type="TEXT",
            enforce_domains="NO_ENFORCE_DOMAINS",
        )
        ...
        print("   remove broadcast burn data for all industry groups except FWS Forestry LLC")
        ### step ? (new code location due to type of org) 
        ...
        select_feature_1 = arcpy.management.SelectLayerByAttribute(
            in_layer_or_view=calc_field_2,
            selection_type="NEW_SELECTION",
            where_clause="AGENCY <> 'Timber Companies' OR AGENCY_ = 'FWS Forestry LLC'"
        )

        select_feature_2 = arcpy.management.SelectLayerByAttribute(
            in_layer_or_view=calc_field_2,
            selection_type="NEW_SELECTION",
            where_clause="AGENCY = 'Timber Companies' AND AGENCY_ <> 'FWS Forestry LLC' AND Burn_Type <> 'Broadcast'"
        )


        arcpy.management.CreateFeatureclass(
            out_path=scratch_workspace, 
            out_name=f"pfirs_no_broadcast", 
            geometry_type="POINT", 
            template=[copy_features_1], 
            spatial_reference=3310, #"NAD 1983 California (Teale) Albers (Meters)"), 
        )

        point_append = arcpy.management.Append(
            inputs=[select_feature_1, select_feature_2], 
            target=os.path.join(scratch_workspace, "pfirs_no_broadcast"), 
            schema_type="NO_TEST", 
            field_mapping="", 
        )

        print("copy_features_1: ", arcpy.management.GetCount(copy_features_1)[0])
        print("select_feature_1: ", arcpy.management.GetCount(select_feature_1)[0])
        print("select_feature_2: ", arcpy.management.GetCount(select_feature_2)[0])
        print("point_append: ", arcpy.management.GetCount(point_append)[0])


        calc_field_3 = arcpy.management.CalculateField(
            in_table=point_append,
            field="ORG_ADMIN_p",
            expression='!AGENCY!',
            expression_type="PYTHON3",
            code_block="",
        ...
        # remove individual timber company names per their wishes

        calc_field_10 = arcpy.management.CalculateField(
            in_table=calc_field_9,
            field="IMPLEMENTING_ORG",
            expression="get_org(!AGENCY_!)",
            expression_type="PYTHON3",
            code_block="""def get_org(agency):
                            if agency in anonymous_org_list:
        ...
        keepfields_1 = KeepFields(output_standardized)
