...
def Model_NFPORS_poly(
    ...
    with arcpy.EnvManager(
        ...
        print("   step 1/7 select by DOI agency (BIA, FWS)")
        select_service = arcpy.analysis.Select(
            in_features=input_original_polys,
            out_feature_class=nfpors_DOI,
            where_clause="agency = 'BIA' Or agency = 'FWS'",
        )

        ######################################################################

        field_list = [field.name for field in arcpy.ListFields(select_service)]   
        print("========>", field_list)
      
        # with arcpy.da.SearchCursor(select_service, ['act_comp_d']) as cursor:
        #    for row in cursor:
        #        print('------>', row[0])
        ...
        select_date_after_1995 = arcpy.analysis.Select(
            ...
            # where_clause="act_comp_d >= '1995-01-01 00:00:00 000' Or act_comp_d IS NULL",
        )
        ...
        calc_field_10 = arcpy.management.CalculateField(
            ...
            #expression="ifelse(!act_comp_d!, !modifiedon!)",
            expression_type="PYTHON3",
