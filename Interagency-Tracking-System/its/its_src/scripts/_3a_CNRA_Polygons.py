...
def CNRA_poly_Model(
    ...
    with arcpy.EnvManager(
        ...
        AddFields(Input_Table=Input_Features_1, alter_fields=True)
        ...
        """
        print([(f.name, f.type) for f in arcpy.ListFields (calc_field_5)])
        MiliSeconds_1 = arcpy.AlterField_management(calc_field_5, "ACTIVITY_END", "ACTIVITY_END_1")
        MiliSeconds_2 = arcpy.management.AddField(MiliSeconds_1, "ACTIVITY_END_1", "DATEONLY")
        MiliSeconds_3 = arcpy.CalculateField_management(MiliSeconds_2, "treatment_end", "!ACTIVITY_END_1!", "PYTHON3")
        ...
        MiliSeconds_6 = arcpy.management.AddField(MiliSeconds_5, "ACTIVITY_START_1", "DATEONLY")
        ...
        field_list_join_1 = [field.name for field in arcpy.ListFields(join_1)]      
        print(f"        join_1: {field_list_join_1}")  
        ...
        field_list_CNRA_Flat_2 = [field.name for field in arcpy.ListFields(CNRA_Flat_2)]      
        print(f"        CNRA_Flat_2: {field_list_CNRA_Flat_2}")  

        Count2 = arcpy.management.GetCount(CNRA_Flat_2)
        print("     CNRA_Flat_2 has {} records".format(Count2[0]))

        field_list_standardized_1 = [field.name for field in arcpy.ListFields(standardized_1)]      
        print(f"        standardized_1: {field_list_standardized_1}")  

        Count_standardized_1 = arcpy.management.GetCount(standardized_1)
        print("     standardized has {} records".format(Count_standardized_1[0]))


        print("   step 10/17 append")
        ## Appending the CNRA table to the table we created ensures the schema is correct
        append_2 = arcpy.management.Append(
            inputs=[standardized_1], 
            target=CNRA_Flat_2, 
            schema_type="NO_TEST", 
            # field_mapping="", 
            # subtype="", 
        ...
        print("     combined has {} records".format(Count2[0]))

        field_list = [field.name for field in arcpy.ListFields(append_2)]      
        print(f"        append before: {field_list }")  
        ...
        field_list_append_2 = [field.name for field in arcpy.ListFields(append_2)]      
        print(f"        append_1: {field_list_append_2}")  
        ...
        calc_field_17 = arcpy.management.CalculateField(
            ...
            code_block="""def ifelse(Stat, Start, End):
                            if (Stat == 'ACTIVE' or Stat == 'Active') and End == None:
                                return datetime.datetime(2027,12,12)
                            if (Stat == 'COMPLETE' or Stat == 'Complete') and End == None:
                                return datetime.datetime(2028,12,12)
                            if (Stat == 'PLANNED' or Stat == 'Planned') and End == None and Start == None:
                                return datetime.datetime(2029,12,12)
        ...
        arcpy.management.CopyFeatures(
            ...
            out_feature_class='activity_end_cnra_poly'
        )


        field_list = [field.name for field in arcpy.ListFields(calc_field_17)]      
