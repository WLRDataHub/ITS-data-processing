...
def Model_USFS(
    ...
    with arcpy.EnvManager(
        ...
        field_list = [field.name for field in arcpy.ListFields(input_fc)]      
        print(f"        field_list: {field_list}")

        Count1 = arcpy.management.GetCount(input_fc)
        ...
        Count1 = arcpy.management.GetCount(select_CA)
        print("Selected Activities have {} records".format(Count1[0]))
        ...
        ### add restrict to code 2510

        select_non_wildfire_5 = arcpy.management.SelectLayerByAttribute(
            in_layer_or_view=select_non_wildfire_4,
            selection_type="SUBSET_SELECTION",
            where_clause="ACTIVITY_CODE = '2510' And FUELS_KEYPOINT_AREA <> '6'",
            invert_where_clause="INVERT",
        )

        select_non_wildfire_6 = arcpy.management.SelectLayerByAttribute(
            in_layer_or_view=select_non_wildfire_5,
            selection_type="SUBSET_SELECTION",
            where_clause="ACTIVITY_CODE = '2510' And FUELS_KEYPOINT_AREA <> '3'",
            invert_where_clause="INVERT",
        )

        select_date_not_null = arcpy.management.SelectLayerByAttribute(
            in_layer_or_view=select_non_wildfire_6,
            selection_type="SUBSET_SELECTION",
        ...
        calc_field_12 = arcpy.management.CalculateField(
            ...
            field="ACTIVID_USER",
            expression="str(!SUID!)+'-'+str(!OBJECTID!)",
            expression_type="PYTHON3",
            code_block="",
            field_type="TEXT",
            enforce_domains="NO_ENFORCE_DOMAINS",
