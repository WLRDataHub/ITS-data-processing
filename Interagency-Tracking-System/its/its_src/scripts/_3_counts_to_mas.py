...
def CountsToMAS(Input_Table):  
    ...
    print("             this step is redundant and therefore skipped")
    """
    select_1 = arcpy.management.SelectLayerByAttribute(
                        ...
                        #where_clause="ACTIVITY_END >= timestamp '2023-01-01 00:00:00' And ACTIVITY_END < timestamp '2024-01-01 00:00:00'"
                        )
    """
    ...
    print("        counts step 6/8: set to 'NO' if Agency is 'Other' and Admin is 'CARB'") # aimed at PFIRS data
    calc_field_6 = arcpy.management.CalculateField(
                        in_table=calc_field_5, 
                        field="COUNTS_TO_MAS", expression="ifelse(!AGENCY!,!TRMTID_USER!,!COUNTS_TO_MAS!)", 
                        code_block="""def ifelse(Agency, TRMTID, Counts):
                            if Agency == 'OTHER' and TRMTID[:5] == 'PFIRS':
                                return 'NO'
                            else:
                                return Counts"""
                                )
    ### per Emily's request, for 2023 ONLY, the State Parks should be excluded from PFIRS
